...
import '../providers/health_metric_providers.dart';
...
class _DataEntryPageState extends ConsumerState<DataEntryPage>
  ...
  Future<void> _saveData(bool isNutrition) async {
    try {
      if (isNutrition) {
        // Nutrition data is handled separately in Food Diary
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please use Food Diary to log nutrition data'),
      ...
      final gkiRepo = ref.read(gkiRepositoryProvider);
      ...
      if (_glucoseController.text.isNotEmpty) {
        ...
        if (glucose != null) {
          await healthRepo.saveHealthMetric(
            HealthMetric(
              id: _uuid.v4(),
              timestamp: now,
      ...
      // Save BHB ketones
      if (_bhbController.text.isNotEmpty) {
        final ketones = double.tryParse(_bhbController.text);
        if (ketones != null) {
          await healthRepo.saveHealthMetric(
            HealthMetric(
              id: _uuid.v4(),
              timestamp: now,
              type: HealthMetricType.bloodKetones,
              value: ketones,
              unit: 'mmol/L',
            ),
          );

          // Calculate and save GKI if both glucose and ketones are available
          if (_glucoseController.text.isNotEmpty) {
            final glucose = double.tryParse(_glucoseController.text);
            if (glucose != null) {
              await gkiRepo.saveGKI(
                id: _uuid.v4(),
                timestamp: now,
                glucose: glucose,
                ketones: ketones,
      ...
      ref.invalidate(latestHealthMetricsProvider);
      ref.invalidate(latestGkiProvider);
      ref.invalidate(recentHealthMetricsProvider(5));
